...
//		connman.manager.RequestScan('wifi');
	...
	this.Connect = function(ssid, passphrase, security) {
		...
		if (!('security' in args)) {
			/* Auto-detect security for this access point */
			var list = connman.manager.GetServices();
			for (var index in list) {
				if (list[index][1].Type == 'wifi' && [index][1].Name == ssid) {
					args.security = list[index][1].Security;
					break;
				}
			}
		}
/*
		connman.manager.ConnectService({
			Type: 'wifi',
			Mode: 'managed',
			SSID: ssid
		}, );
